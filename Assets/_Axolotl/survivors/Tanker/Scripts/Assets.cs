...
namespace Axolotl.Tanker.Modules
{
    internal static class Assets
    {
		//The file name of your asset bundle
		internal const string assetBundleName = "Tankerbundle";

		//Should be the same name as your SerializableContentPack in the asset bundle
		internal const string contentPackName = "TankerContentPack";
        ...
        internal static void Init()
        {
            ...
            {
                Debug.LogError(TankerPlugin.MODNAME + ": AssetBundle name hasn't been changed. Not loading any assets to avoid conflicts.");
				TankerPlugin.cancel = true;
                return;
            }
		...
		internal static void PopulateAssets()
		{
			if (!mainAssetBundle)
			{
				Debug.LogError(TankerPlugin.MODNAME + ": AssetBundle not found. Unable to Populate Assets.");
				TankerPlugin.cancel = true;
				return;
			}
		}
		...
		internal static void LoadAssetBundle()
        {
            if (mainAssetBundle == null)
            {
				...
				if (!mainAssetBundle) 
				{
					Debug.LogError(TankerPlugin.MODNAME + ": AssetBundle not found. File missing or assetBundleName is incorrect.");
					TankerPlugin.cancel = true;
					return;
				}
				LoadContentPack();
        ...
        internal static void LoadSoundBank()
        {
			...
			{
				Debug.LogError(TankerPlugin.MODNAME + ": SoundBank name hasn't been changed - not loading SoundBank to avoid conflicts.");
				return;
			}

			if (soundBankName == "")
            {
				Debug.LogFormat(TankerPlugin.MODNAME + ": SoundBank name is blank. Skipping loading SoundBank.");
				return;

			}

			using (Stream manifestResourceStream2 = Assembly.GetExecutingAssembly().GetManifestResourceStream(TankerPlugin.MODNAME + "." + soundBankName +".bnk"))
			{
